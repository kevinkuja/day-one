...
contract PriceEngineTest is Test {
  ...
  function setUp() public {
    ...
    priceEngine = new PriceEngine(address(yieldPlatform), address(factory), owner);
    vm.prank(owner);
  ...
  function testgetPriceMetricsDown() public {
    address token = payable(factory.artistToToken(artist1));
    ...
    vm.prank(user);
    ArtistToken(payable(token)).mint{value: cost}(user, amount);
  ...
  function testgetPriceMetricsUpYieldOnly() public {
    vm.startPrank(owner);
    address token1 = payable(factory.artistToToken(artist1));
    priceEngine.setMetrics(artist1, 100);
    address token2 = payable(factory.artistToToken(artist2));
    ...
    ArtistToken(payable(token1)).mint{value: cost}(user, amount);
    ArtistToken(payable(token2)).mint{value: cost}(user, amount);
